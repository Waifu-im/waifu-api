...
// Get version from assembly (set via /p:Version in CI)
var rawVersion = typeof(Program).Assembly
    .GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion ?? "0.0.0";
// Strip the +commit hash suffix (e.g., "7.0.0+abc123" -> "7.0.0")
var apiVersion = rawVersion.Split('+')[0];
   ...
   options.AddDocumentTransformer((document, context, cancellationToken) =>
    {
        ...
        var apiBaseUrl = builder.Configuration[ConfigurationKeys.ApiBaseUrl]!;
        var serverUrl = !string.IsNullOrEmpty(basePath) && basePath != "/"
            ? $"{apiBaseUrl.TrimEnd('/')}{basePath}"
            : apiBaseUrl;
