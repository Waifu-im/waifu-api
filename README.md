...
The API version is managed automatically using [MinVer](https://github.com/adamralph/minver) based on Git tags with a `v` prefix.

**How it works:**
- Tags must use the `v` prefix (e.g., `v7.0.0`)
- Between tags, MinVer auto-increments the patch version and adds a pre-release suffix
- Example: After `v7.0.0`, commits are versioned as `7.0.1-alpha.0.1`, `7.0.1-alpha.0.2`, etc.
...
**Override version manually (CI/CD or local):**
```bash
dotnet build -p:MinVerVersionOverride=7.0.0
```

The version is displayed in the OpenAPI documentation at `/openapi/v1.json`.
