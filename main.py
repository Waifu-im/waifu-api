...
async def is_valid_token(token_header,request_perms=None):
    if not token_header:
        return quart.abort(401,description="No Token, please check that you provided a Token and that your correctly formated it in the Authorization header.")
...
async def principal(typ,categorie):
    ...
    if typ in autho:        
        ...
        print(f"Total : {end-start} Argument : {database-start} Database : {jsonformating-database} Formating : {end-jsonformating}")
        if not tagsjson:
        ...
        print(f"Files : {[im['file']+im['extension'] for im in tagsjson[0]['images']]}")
        return jsonify(code=200,tags=tagsjson,url='https://api.waifu.im/image/oldjson.png')
...
async def endpoints_():
    ...
    data=await myendpoints_info(app,over18=None) if request.args.get("full",type=convert_bool) else await myendpoints(app,over18=None)
    return jsonify(data)
...
async def log_request(response):
    """Add some informations such as ip adress and User-Agent to the database (for statistics purpose) no need to worry."""
    if response.status_code==200:
        await app.pool.execute("INSERT INTO api_logs(remote_address,url,user_agent) VALUES($1,$2,$3)",request.remote_addr,request.url,request.user_agent.string)
    return response
